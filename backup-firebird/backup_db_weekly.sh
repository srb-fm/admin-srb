...
function f_check_mountz () {
	...
	if ping -c 1 $mount_ip &> /dev/null; then
		if grep -qs "$mount_name" /proc/mounts; then
			echo "It's mounted."
			mount_available="Y"
			echo "mounted: $mount_name" >> $fb_db_backup_log
		else
			...
			if [ $? -eq 0 ]; then
				echo "Mount success!"
				mount_available="Y"
				echo "now mounted: $mount_name" >> $fb_db_backup_log
			else
				echo "Something went wrong with the mount..."
				echo "$mount_name not mounted" >> $fb_db_backup_log
			fi
		fi
	else
		echo "Not available: $mount_ip"
		echo "$mount_ip not available" >> $fb_db_backup_log
	fi
}

running=$(date +'%Y-%m-%d-%H-%M-%S')
echo "$running running backup_db_weekly.sh..."
echo "$running running backup_db_weekly.sh..." >> $fb_db_backup_log
...
if [ $mount_available == "Y" ]; then
	...
	echo "finish backup_db_weekly.sh..." >> $fb_db_backup_log
else
