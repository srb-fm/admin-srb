...
Dieses Script registriert ausgespielte Beitraege in der Datenbank und uebertraegt sie auf den Webserver.
Dort koennen die  zur Anzeige des aktuellen Beitrags/Titels und fuer Playlisten genutzt werden.

Dateiname Script: play_out_logging.py
Schluesselworte fuer Einstellungen: PO_Logging_Config_1/ PO_Switch_Broadcast_Config_1
...
2. Form aufrufen und im Intervall Metadaten der Beitraege/Titel ermitteln und in den DBs speichern 
...
Details:
Die Quellen werden aus dem Logeintrag (Sendequellenumschalter) in der DB ermittelt.
Wenn Aussenuebertragung, dann gibts keine Logfiles, also Sendedaten aus db hohlen.
Wenn VP oder Studio, dann die mAirlist-Logdateien einlesen und mit db vergleichen.
Zu guter letzt, Daten in db schreiben und auf Webserver uebertragen.
Bei Fehlern gibt es ein Fallback. 
Es wird der in der Config voreingestellte Autor und Titel geloggt und uebertragen.
...
Error 003 Webserver fuer PlayOut-Logging lieferte bei Uebertragung Fehler zurueck
Error 004 Webserver fuer PlayOut-Logging nicht erreichbar

Parameterliste:
Param 1: mAirList Logdatei aktueller Titel 
(Erweiterung um Nummer und Endung "log" wird durch das logging-Programm vorgenommen)
...
Param 6: Benutzer
Param 7: Passwort
Param 8: mAirList Logdatei der Studio-Rechner, aktueller Titel (Erweiterung um Nummer und Endung "log" wird durch das logging-Programm vorgenommen)
...
koennen bei laengerer Laufzeit zu Speicherproblemen fuehren, besonders unter Windows.

Man bedenke:
Und dein Knecht steht mitten in deinem Volk, das du erwaehlt hast, einem Volk, so gross, 
dass es wegen seiner Menge niemand zaehlen noch berechnen kann. Die Bibel 1. Koenige 3,8
"""
...
class app_config( object ):
    ...
    def __init__(self):
        ...
        self.app_errorslist.append(u"Error 001 Sende-Quelle kann aus Datenbank nicht ermittelt werden ")
        self.app_errorslist.append(u"Error 002 Play-Out-Log-Datei kann nicht gelesen werden")
        self.app_errorslist.append(u"Error 003 Webserver fuer PlayOut-Logging lieferte bei Uebertragung Fehler zurueck")
        self.app_errorslist.append(u"Error 004 Webserver fuer PlayOut-Logging nicht erreichbar")
        self.app_errorslist.append(u"Error 005 Externes PlayOut-Logging ausgesetzt, Webserver nicht erreichbar")
...
def extract_from_stuff( stuff,  match_string_1, offset_1,  match_string_2,  offset_2  ):    
...
def upload_data_prepare():
    ...
    data_upload = {'pa' : 'hinein',
        ...
        'pd' : c_autor,
        'pe' : c_title,
        'pf' : ac.config_extended[6],
        'pg' : ac.config_extended[7]}
    ...
    try:
        ...
        #print log_message 'feuerpfeil'  'xyz'
        db.write_log_to_db(ac, log_message, "x")
        data_upload = {'pa' : 'hinein',
            'pb' : ac.log_start,
            'pc' : ac.log_start,
            'pd' : ac.config_extended[3],
            'pe' : ac.config_extended[4],
            'pf' : ac.config_extended[6],
            'pg' : ac.config_extended[7]}
        
        lib_cm.message_write_to_console(ac, u"data_upload" + c_autor + " " + c_title )
        data_upload_encoded = urllib.urlencode( data_upload )
...
def work_on_data_from_logfile(time_now,  log_data):
    ...
    if re.match("\d{7,}",log_filename ) is not None:
        lib_cm.message_write_to_console(ac, u"srb_sendung_id_in_title: daten aus db, erster versuch mit zeit und id" )
        # id extrahieren und sendung in db suchen
        sendung_id =  log_filename[0 : 7]
        db_tbl_condition = ("A.SG_HF_ON_AIR = 'T' AND SUBSTRING(A.SG_HF_TIME FROM 1 FOR 10) = '" + str( time_now.date() )+ "' " 
            "AND SUBSTRING(A.SG_HF_TIME FROM 12 FOR 2) = '" + str( time_now.hour ) + "' AND B.SG_HF_CONT_ID ='" + sendung_id + "'" )
        sendung_data =  db.read_tbl_row_sg_cont_ad_with_cond(ac, db, db_tbl_condition)
        if sendung_data is None:
            sendung_data_search_for_id_only = "yes"

    if sendung_data_search_for_id_only == "yes":
        lib_cm.message_write_to_console(ac, u"srb_sendung_id_in_title: daten aus db, zweiter versuch nur mit id" )
        db_tbl_condition = "A.SG_HF_ON_AIR = 'T' AND B.SG_HF_CONT_ID ='" + sendung_id + "'" 
        sendung_data =  db.read_tbl_row_sg_cont_ad_with_cond(ac, db, db_tbl_condition)
    ...
    if sendung_data is not None:
        ...
        if author_title_ok == "yes":
            ...
            #print log_title
            # split in autor und title, klappt nur wenn ein unterstrich dazwischen
...
class my_form( Frame ):
    ...
    def createWidgets(self):
        ...
        self.text_label = Label(self, height=1, width=80, anchor=NW,  text="Play-Out-Logging Nr: ")
        self.text_label.pack()
        
        self.text_label_1 = Label(self, height=1, width=80, anchor=NW,  text="Titel aktuell")
        ...
        self.text_label_2 = Label(self, height=1, width=80, anchor=NW,   text="Rueckmeldung von Webserver")
    ...
    def display_logging(self, log_meldung_1, log_meldung_2):
        ...
        if log_meldung_1 is not None:
            self.text_label.config( text="Play-Out-Logging Nr: " + str( ac.app_counter ) + "  / Interval von " + str( int( ac.config_extended[2])) + " Sekunden"  )
        ...
        self.listenID = self.after( int( ac.config_extended[2] )*1000, self.lets_rock)
        return
    ...
    def lets_rock( self  ):
        ...
        #source_log = db.read_tbl_row_with_cond(ac,  db, "USER_LOGS",  "USER_LOG_ACTION, USER_LOG_TIME", u"USER_LOG_ACTION LIKE 'Datei für Sendequellenumschalter geschrieben:%' AND SUBSTRING( USER_LOG_TIME FROM 1 FOR 13) ='" +  c_time + "' ORDER BY USER_LOG_TIME DESC" )
        source_log = db.read_tbl_row_with_cond_log(ac,  db, "USER_LOGS",  "USER_LOG_ACTION, USER_LOG_TIME", u"USER_LOG_ACTION LIKE 'Datei für Sendequellenumschalter geschrieben:%' AND SUBSTRING( USER_LOG_TIME FROM 1 FOR 13) ='" +  c_time + "' ORDER BY USER_LOG_TIME DESC" )
        ...
        if source_id == "03":
            file_mairlist_log = ac.config_extended[1] + "_" + source_id + ".log"
        else:
            file_mairlist_log = ac.config_extended[8] + "_" + source_id + ".log"
        lib_cm.message_write_to_console(ac, file_mairlist_log)
        
        # 3. Pruefen ob Quelle Aussenuebertragung, VP oder Studios
        # Bei Aussenuebertragung stehen keine Logfiles zur Verfuegung, Sendung muesste in db zu finden sein
        if source_id == "05":
            lib_cm.message_write_to_console(ac, u"ISDN-Uebertragung")  
            # Daten aus db holen
            db_tbl_condition = ("A.SG_HF_ON_AIR = 'T' AND SUBSTRING(A.SG_HF_TIME FROM 1 FOR 10) = '" + str( time_now.date() )+ "' " 
                "AND SUBSTRING(A.SG_HF_TIME FROM 12 FOR 2) = '" + str( time_now.hour ) + "' AND A.SG_HF_SOURCE_ID ='" + source_id + "'" )
            sendung_data = db.read_tbl_row_sg_cont_ad_with_cond(ac, db, db_tbl_condition)
            if sendung_data is not None:
                if ac.log_start == str( sendung_data[2] ):
                    ...
                    # Daten der Sendung fuer Vergleich bei naechstem Durchlauf einlesen 
                    ac.log_start = str( sendung_data[2] )
                    ac.log_author = sendung_data[12] + " " + sendung_data[13]
                    ac.log_title = sendung_data[9] 
            else:
                lib_cm.message_write_to_console(ac, u"ISDN-Uebertragung, Sendung nicht in DB gefunden" )
            ...
            lib_cm.message_write_to_console(ac, u"Sendung aus Studios, Playout oder Internetstream" ) 
            # Daten aus mAirlist_Logdatei holen
            mairlist_log_data = lib_cm.read_file_first_line(ac, db, file_mairlist_log)
            ...
            if mairlist_log_data is None:
                ...
                if ac.error_counter_read_log_file == 1: 
                    ...
                    ac.log_author = ac.config_extended[3]
                    ac.log_title = ac.config_extended[4]
            ...
            # bei direktem Vergleich des Inhalts der Logdatei (mairlist_log_data) funktioniert folgender if-Vergleich nicht aussserhalb der ide, deshalb in vari
