...
Param 4: Pfad vom Server zu Protokolldateien (gleich wie in PO_Protokoll_Config_3)
...
class app_config( object ):
    ...
    def __init__(self):
        ...
        self.app_errorslist.append(u"Error 001 Fehler beim Kopieren der Protokolldatei in Play-Out")
...
def load_sg_repeat(ac):
    ...
    # log schreiben
    log_message = u"Wiederholungssendungen aus Protokoll noetig für: " + str(ac.time_target.date()) 
...
def check_and_work_on_files(repeat_sendung):
    ...
    try:
        ...
        # Ergebnis des split repeat_sendung[12].split("_")[0]) ist unicode, repeat_sendung[8] ist jedoch int, 
    ...
    try:
        # der versuch den 2. splitt zu bekommen gibt fehler wenn split-muster nicht gefunden
    ...
    file_dest = path_dest + str(repeat_sendung[8]) + "_" + lib_cm.replace_uchar_sonderzeichen_with_latein(repeat_sendung[16]) + "_" + lib_cm.replace_uchar_sonderzeichen_with_latein(repeat_sendung[13]) + ".mp3"
    #file_dest = str(repeat_sendung[8]) + "_" + repeat_sendung[16] + "_" + repeat_sendung[13] + ".mp3"
    ...
    db.write_log_to_db(ac, u"Erstsendung zu Wiederholungssendung gefunden: " + first_sg_date_time.strftime('%Y_%m_%d_%H') + " " + repeat_sendung[11], "t")
    ...
    if ac.app_windows == "yes":
        file_source = path_source + first_sg_date_time.strftime('%Y_%m_%d') + "\\" + db.ac_config_1[5] + "_" + first_sg_date_time.strftime('%Y_%m_%d_%H') + ".mp3"
    else:
        file_source = path_source + first_sg_date_time.strftime('%Y_%m_%d') + "/" + db.ac_config_1[5] + "_" + first_sg_date_time.strftime('%Y_%m_%d_%H') + ".mp3"
    ...
    # audio-fx
    audio_validate(ac, file_dest)
    audio_mp3gain(ac, file_dest)
    audio_id3tag(ac, file_dest,  lib_cm.replace_uchar_sonderzeichen_with_latein(repeat_sendung[16]), lib_cm.replace_uchar_sonderzeichen_with_latein(repeat_sendung[13]) )
    ...
    if db_op_success is not None:
       lib_cm.message_write_to_console(ac, "Datensatz aktualisiert: " + filename)
       db.write_log_to_db(ac,  u"Dateiname in Sendeanmeldung aktualisiert: " + filename, "k"  )
...
def lets_rock():
    ...
    if repeat_sendungen is None:
        db.write_log_to_db(ac, u"Zur Zeit keine Wiederholungssendungen aus Protokoll vorgesehen", "t")
        return
    ...
    for item in repeat_sendungen:
        ...
        if first_sendung is not None:
            ...
            if first_sendung[0][2]+ datetime.timedelta(hours=+1 ) <= datetime.datetime.now() :
               lib_cm.message_write_to_console( ac, "Erstsendung vor mindestens einer Stunde gelaufen oder beendet, Proto muss vorhanden sein" )
               ...
               repeat_offline = check_and_work_on_files(item)
                ...
                lib_cm.message_write_to_console( ac, first_sendung[0][2] )
                lib_cm.message_write_to_console( ac, datetime.datetime.now() )
                db.write_log_to_db(ac, first_sendung[0][11] + u": Erstsendung zu WH noch nicht gelaufen, Weiterverarbeitung abgebrochen...", "t")
