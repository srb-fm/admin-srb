...
class app_config( object ):
    ...
    def __init__(self):
        ...
        self.app_config_develop = u"PO_DB_Config_3_e"
        # anzahl parameter
        ...
        self.app_errorslist.append(u"Error 001 beim Generieren des Stichwortes fuer duplizierte Sendung")
        self.app_errorslist.append(u"Error 002 Duplizierte Sendung konnte nicht gebucht werden ")
        self.app_errorslist.append(u"Error 003 beim Loeschen der duplizierten Sendung (Eintrag der Meta-Daten ist fehlgeschlagen) ")
        ...
        self.time_target_start = datetime.datetime.now() + datetime.timedelta( days=-2 ) + datetime.timedelta( hours=-10 )
        self.time_target_end = datetime.datetime.now() + datetime.timedelta(days=+4 ) + datetime.timedelta(hours=+4 )
        # develop at Tuesday
        self.time_target_start = datetime.datetime.now() + datetime.timedelta( days=-1 ) + datetime.timedelta( hours=-10 )
        self.time_target_end = datetime.datetime.now() + datetime.timedelta(days=+5 ) + datetime.timedelta(hours=+4 )
        #self.time_target = datetime.datetime.now()

def search_sg(ac, sg_titel,  t_sg_time ):
    """Pruefen ob geplante Sendebuchung schon vorhanden"""
    lib_cm.message_write_to_console( ac, u"Pruefen ob geplante Sendebuchung schon vorhanden" )
    db_tbl_condition = "A.SG_HF_FIRST_SG ='T' AND SUBSTRING(A.SG_HF_TIME FROM 1 FOR 19) = '" + str( t_sg_time )+ "' " + "AND B.SG_HF_CONT_TITEL='"+ sg_titel +"' "
...
def load_roboting_sgs(ac):
    ...
    sendungen_data = db.read_tbl_rows_with_cond(ac,  db,  "SG_HF_ROBOT", "SG_HF_ROB_TITEL, SG_HF_ROB_STICHWORTE", "SG_HF_ROB_DUB ='T'" )
...
def load_sg(ac, sg_titel ):
    ...
    db_tbl_condition = ("A.SG_HF_FIRST_SG ='T' "
        "AND SUBSTRING(A.SG_HF_TIME FROM 1 FOR 19) >= '" + ac.time_target_start.strftime("%Y-%m-%d %T") + "' " 
        "AND SUBSTRING(A.SG_HF_TIME FROM 1 FOR 19) <= '" + ac.time_target_end.strftime("%Y-%m-%d %T") + "' " 
        "AND B.SG_HF_CONT_TITEL='"+ sg_titel +"' ")
    sendung_data =  db.read_tbl_rows_sg_cont_ad_with_cond_3(ac, db, db_tbl_condition)

    if sendung_data is None:
        log_message = u"Keine Erst-Sendung mit diesem Titel in diesem Zeitraum gefunden: " + sg_titel + " - " + ac.time_target_start.strftime("%Y-%m-%d %T") + " bis " + ac.time_target_end.strftime("%Y-%m-%d %T") 
        db.write_log_to_db_1(ac,  log_message, "e", "write_also_to_console" )
        return sendung_data
    return sendung_data

def write_sg_main(ac, l_data_sg_main ):
    """Sendung Main eintragen"""
    lib_cm.message_write_to_console( ac, u"Sendung eintragen" )
    sql_command_sg_main = ("INSERT INTO SG_HF_MAIN( SG_HF_ID, SG_HF_CONTENT_ID, SG_HF_TIME, "
            "SG_HF_DURATION, SG_HF_INFOTIME, SG_HF_MAGAZINE, SG_HF_PODCAST, SG_HF_ON_AIR, "
            "SG_HF_SOURCE_ID, SG_HF_REPEAT_PROTO, SG_HF_FIRST_SG)"
            " values ('" + str(l_data_sg_main[0]) + "', '" + str(l_data_sg_main[1]) + "', '" + str(l_data_sg_main[2]) + "', '" +
            l_data_sg_main[3] + "', '" + l_data_sg_main[4] + "', '" + l_data_sg_main[5] + "', '" + l_data_sg_main[6] + "', '" + 
            l_data_sg_main[7] + "', '" + l_data_sg_main[8] + "', '" + l_data_sg_main[9] + "', '" + l_data_sg_main[10]
            + "')")
...
def write_sg_cont(ac, l_data_sg_content ):
    """Sendung Content eintragen"""
    sql_command_sg_cont = ("INSERT INTO SG_HF_CONTENT( SG_HF_CONT_ID, SG_HF_CONT_SG_ID, "
        ...
        " values ('" + str(l_data_sg_content[0]) + "', '" + str(l_data_sg_content[1]) + "', '" + str(l_data_sg_content[2]) + "', '" +
        l_data_sg_content[3] + "', '" + l_data_sg_content[4] + "', '" + l_data_sg_content[5] + "', '" + l_data_sg_content[6] + "', '" + 
        l_data_sg_content[7] + "', '" + l_data_sg_content[8] + "', '" + l_data_sg_content[9] + "', '" + l_data_sg_content[10]+ "', '" + 
...
def delete_failed_sg_in_db(ac, main_id_sg ):
    """In SG_MAIN wieder loeschen wenn SG_CONT fehlgeschlagen"""
    lib_cm.message_write_to_console( ac, u"delete_failed_sg_in_db with nr: " + str(main_id_sg) )
    ACTION = "DELETE FROM SG_HF_MAIN WHERE SG_HF_ID = " + str(main_id_sg) + " ROWS 1"
    db.dbase_connect( ac )
    if db.db_con is None:
        err_message = log_message + u"Error General: No connection to db, delete_failed_sg_in_db: %s" % str(e)
...
def lets_rock():
    ...
    for item in roboting_sgs:
        ...
        db.write_log_to_db_1(ac, u"Sendung zum Duplizieren gefunden: "+sendung[0][14].encode('ascii', 'ignore'), "t", "write_also_to_console" )
        ...
        lib_cm.message_write_to_console(ac, sendung[0][2] + datetime.timedelta(days=+7 ) )
        lib_cm.message_write_to_console(ac, ac.time_target_start.date() )
        lib_cm.message_write_to_console(ac, ac.time_target_start.strftime("%Y-%m-%d %T") )
        ...
        l_data_sg_main = [main_id_sg, main_id_sg_cont, dt_sg_new_date, sendung[0][3], sendung[0][4], sendung[0][5], sendung[0][6], 
            u"F", sendung[0][8], sendung[0][9], sendung[0][10]]
        ...
        try:
            ...
            lib_cm.message_write_to_console(ac, "Fehler Stichwort generieren :" + str(e) )
        ...
        if sendung[0][8] == "03":
            ...
            if sendung[0][15][0:7] == "http://":
                ...
                sg_filename = str(main_id_sg_cont) + "_" + lib_cm.replace_uchar_sonderzeichen_with_latein(sendung[0][25]) + "_" + lib_cm.replace_uchar_sonderzeichen_with_latein(sg_stichwort) + ".mp3"
        else:
            # Live
            sg_filename = "Keine_Audiodatei"
        
        # values l_data_sg_content aus gefundener sendung zusammenbauen
        l_data_sg_content = [main_id_sg_cont, main_id_sg, sendung[0][13], sendung[0][14], sg_filename, sg_stichwort, sendung[0][17], 
            sendung[0][18], sendung[0][19], sendung[0][20], sendung[0][21], sendung[0][22]]
        ...
        if db_op_success_main is not None:
            ...
            if db_op_success_cont is None:
                ...
                lib_cm.message_write_to_console(ac, "Fehler bei insert into sg_cont, main_sg wieder loeschen! " + sendung[0][14].encode('ascii', 'ignore'))
        ...
        if db_op_success_cont is not None:
            #db.write_log_to_db_1(ac, "automatisch gebucht, bitte pruefen: " + sendung[0][14] + " " + str(dt_sg_new_date), "i", "write_also_to_console" )
            db.write_log_to_db_1(ac, "automatisch gebucht, bitte pruefen: " + sendung[0][14].encode('ascii', 'ignore') + " " + str(dt_sg_new_date), "i", "write_also_to_console" )
            time.sleep( 1 )
            #db.write_log_to_db_1(ac, "automatisch gebucht, bitte pruefen: " + sendung[0][14] + " " + str(dt_sg_new_date), "n", "write_also_to_console" )
            db.write_log_to_db_1(ac, "automatisch gebucht, bitte pruefen: " + sendung[0][14].encode('ascii', 'ignore') + " " + str(dt_sg_new_date), "n", "write_also_to_console" )
            time.sleep( 2 )
    ...
    if db.ac_config_1 is not None:
        ...
        if param_check is not None:
            if db.ac_config_1[1] == "on":
                ...
                db.write_log_to_db_1(ac, "Sendungs-Dublikator ausgeschaltet", "e", "write_also_to_console" )
