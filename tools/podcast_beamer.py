...
Das Script wird zeitgesteuert zwischen 6 und 20 Uhr jeweils zu Minute 15 ausgefuehrt.
...
class app_config( object ):
    ...
    def __init__(self):
        ...
        self.app_errorslist.append(u"Error 001 Fehler beim Verbinden zum Podcast-ftp-Server")
        self.app_errorslist.append(u"Error 002 Fehler beim Recodieren der Podcast-mp3-Datei")
        self.app_errorslist.append(u"Error 003 Recodierte Podcast-mp3-Datei nicht gefunden")
        self.app_errorslist.append(u"Error 004 Fehler beim Loeschen der Temp-Podcast-Datei")
...
def load_podcast(ac):
    ...
    # log schreiben
    log_message = u"Podcast-Sendungen vorhanden für: " + str(ac.time_target.date()) 
    db.write_log_to_db(ac, log_message, "t")
    return sendung_data
...
def encode_file(podcast_sendung):
    ...
    c_id3_title_value_uni = lib_cm.replace_sonderzeichen_with_latein( podcast_sendung[1] )
    ...
    #c_id3_author_value = podcast_sendung[2].encode( ac.app_encode_out_strings ) + " " + podcast_sendung[3].encode( ac.app_encode_out_strings )
    c_id3_author_value_uni = lib_cm.replace_sonderzeichen_with_latein( podcast_sendung[2] ) + " " + lib_cm.replace_sonderzeichen_with_latein( podcast_sendung[3] )
    c_id3_author_value = c_id3_author_value_uni.encode( ac.app_encode_out_strings )
    ...
    c_source_file = path_sendung_source.encode( ac.app_encode_out_strings ) + podcast_sendung[0].encode( ac.app_encode_out_strings )
    ...
    if podcast_sendung[4] =="T":
        ...
        c_source_file = path_it_mg_source.encode( ac.app_encode_out_strings ) + podcast_sendung[0].encode( ac.app_encode_out_strings )
    ...
    c_dest_file = path_dest.encode( ac.app_encode_out_strings ) + podcast_sendung[0].encode( ac.app_encode_out_strings )
    ...
    # das geht auch
    #p = subprocess.Popen([c_lame_encoder, "--add-id3v2",  c_id3_title, c_id3_title_value, c_id3_author,  c_id3_author_value,  c_source_file, c_dest_file ]).communicate()
    # ausgaben abfangen
    ...
    p = subprocess.Popen([c_lame_encoder, u"--add-id3v2".encode( ac.app_encode_out_strings ),  c_id3_title, c_id3_title_value, c_id3_author,  c_id3_author_value,  c_source_file, c_dest_file ],  stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate(  )
    #print p
...
def check_files_online (podcast_sendungen):
    ...
    try:
        ...
        lib_cm.message_write_to_console(ac, u"ftp: no connect to: " + db.ac_config_1[8])
        return None
    ...
    try:
        ...
        if str(resp) == "550 No files found":
            lib_cm.message_write_to_console( ac, u"ftp: no files in this directory" )
        else:
            raise    
    ...
    #liste enthaelt alle online-dateien, nun audiodateien herausfiltern (nummernblock am anfang)
    files_online_1 = []
    for item in files_online:
        #if ( re.match("[0-9]{7}",  item ) != None ):
...
def upload_file(podcast_sendung):
    ...
    if os.path.isfile( c_source_file ):
        ...
        lib_cm.message_write_to_console( ac, u"upload_files: " + "%r: %s" % ( msg,  c_source_file ) )
